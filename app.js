...
$(document).ready(function() {
     $.ajax({
        type: "GET",
        url: "http://127.0.0.1:8887/validation_tool/wiki.csv",
        dataType: "text",
        success: function(data) {
                    processData(data, lines);
                    $.ajax({
                        type: "GET",
                        url: "http://127.0.0.1:8887/validation_tool/wiki_result.csv",
                        dataType: "text",
                        success: function(data) {
...
function showNext(last_idx){
    ...
    for (let i = last_idx; i < lines.length && counter < 20; i++, counter++) {
        let current = lines[i];
        let age = parseInt(current[2])
        if (age > 20) {
            counter--;
            continue;
        }
        let gender = parseFloat(current[3]);
        let imgname = "/Age-Gender-Predictor/" + current[0] + "_aligned/" + current[1].slice(2, -2);
        var row = "<tr>";
